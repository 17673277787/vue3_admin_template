        ...
        <el-form-item label="销售属性">
            <el-form :inline="true">
                <el-form-item v-for="(item) in saleArr" :key="item.id" :label="item.saleAttrName">
                    <el-select style="width: 200px" v-model="item.saleAttrIdAndValueId">
...
let imgArr = ref<any>([]);
//收集SKU的参数
//ref返回的是一个响应式数据，reactive返回的是一个普通对象
...
const save = async () => {
    //整理参数，平台属性
    let attrArrs = attrArr.value.reduce((pre: any, item: any) => {
        if (item.attrIdAndValueId) {
            let [attrId, valueId] = item.attrIdAndValueId.split(':');
            pre.push({ attrId, valueId });
        }
        return pre;
    }, []);//这里的reduce方法参数是pre和item，分别是上一次的值和当前的值，reduce的作用是将上一次的值和当前的值进行操作，最终返回一个结果
    skuParams.skuAttrValueList = attrArrs;
    //整理参数，销售属性
    let saleArrs = saleArr.value.reduce((pre: any, item: any) => {
    ...
    console.log('默认图片', skuParams.skuDefaultImg);
    //添加SKU请求
